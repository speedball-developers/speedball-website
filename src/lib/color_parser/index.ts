export default function colorParserToHtml(
	input: string,
	stripColors = false,
	stripLinks = false,
	stripTags = '',
	...
	const COLORPARSER = (function () {
		...
		const constants = {
			HTML_DOLLAR: '&#36;',
			HTML_WHITESPACE: '&#160;', // use &#160 instead of &nbsp to avoid conflicts with toUpperCase $t
		...
		// object which contains all public letiables (which are meant to be exposed to the outside later)
		const pub = {};
		pub.game = 'TMF';
		pub.shadowing = false;
		...
		function parseLinks(string: string, showLinks = true) {
			...
			for (let i = 1; i < chunks.length; i++) {
				...
				if (!end) {
					chunks[i] =
						...
						fixURL(chunks[i]) +
						'">' +
						chunks[i].replace('http://', '').replace('https://', '') +
						'</a>';
		...
		function parseManialinks(string: string, showLinks = true) {
			...
			for (let i = 1; i < chunks.length; i++) {
				...
				if (captured) {
					...
					switch (pub.game) {
						case 'TMF':
							chunks[i] =
								'<a href="' +
								constants.MANIALINK_PREFIX_TMF +
								captured[1] +
								'">' +
								captured[2] +
								'</a>';
				...
				if (!end) {
					switch (pub.game) {
						...
						case 'MP':
							chunks[i] =
								'<a href="' + constants.MANIALINK_PREFIX_MP + chunks[i] + '">' + chunks[i] + '</a>';
							break;
					}
		...
		pub.setShadowing = function (boo: boolean) {
			if (boo) {
				pub.shadowing = true;
			} else {
				pub.shadowing = false;
		...
		pub.toHTML = function (
			...
			const clipboard = { color: false, wide: false, narrow: false, caps: false, italic: false };
	...
	COLORPARSER.setShadowing(true); // enable auto shadowing

	return COLORPARSER.toHTML(input, stripColors, stripLinks, stripTags, replaceWhitespace);
}
